    ...
    <div className="space-y-12">
        ...
        <div key={dateKey} className="space-y-4">
          <div className="flex items-center justify-between px-4 border-b border-border/10 pb-2">
            ...
            <span className="text-[11px] font-bold text-muted-foreground/40 italic">
              — Total: Rp {txs.reduce((sum, t) => sum + (t.type === 'expense' ? Number(t.amount) : 0), 0).toLocaleString('id-ID')}
            </span>
          </div>
            ...
            {txs.map((tx) => (
              <div 
                ...
                className={`group relative bg-white dark:bg-card border border-border/40 rounded-[2.5rem] overflow-hidden transition-all duration-300 ${
                  expandedId === tx.id ? "ring-2 ring-[#9fe870] shadow-2xl scale-[1.02]" : "hover:shadow-lg hover:scale-[1.01]"
                 }`}
              >
                {/* Selection Overlay */}
                {onSelectionChange && (
                  <div 
                    ...
                    onClick={(e) => {
                      ...
                      if (isSelected) {
                        onSelectionChange(selectedIds.filter(id => id !== tx.id));
                      } else {
                        onSelectionChange([...selectedIds, tx.id]);
                      }
                    }}
                  >
                    <div className={`h-full w-full ${selectedIds.includes(tx.id) ? "bg-[#9fe870]" : "bg-transparent group-hover:bg-muted/30"}`} />
                  </div>
                )}
                ...
                >
                  ...
                  {onSelectionChange && (
                    ...
                    >
                      {selectedIds.includes(tx.id) && <div className="w-2.5 h-2.5 bg-[#163300] rounded-sm" />}
                    </div>
                  )}
                  ...
                  <div className="w-10 h-10 sm:w-14 sm:h-14 shrink-0 rounded-xl sm:rounded-[1.5rem] bg-muted/30 flex items-center justify-center transition-all duration-500 group-hover:rotate-6 group-hover:scale-110">
                    <CategoryIcon iconName={getCategoryIcon(tx)} className="w-5 h-5 sm:w-6 sm:h-6 text-foreground/70" />
                  </div>
                  ...
                  <div className="flex-1 min-w-0 pr-2">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <Badge variant="secondary" className="bg-muted/50 text-[8px] sm:text-[9px] font-medium uppercase tracking-widest px-2 py-0.5 sm:px-2.5 sm:py-0.5 rounded-full border-none">
                        {tx.category}
                      </Badge>
                    </div>
                  </div>
                  ...
                  <div className="text-right shrink-0">
                    <p className={`font-black text-sm sm:text-xl tracking-tighter ${tx.type === "income" ? "text-emerald-500" : "text-rose-500"}`}>
                      {tx.type === "income" ? "+" : "-"}{formatCurrency(tx.amount)}
                    </p>
                  </div>

                  <div className={`ml-1 sm:ml-2 shrink-0 transition-transform duration-300 ${expandedId === tx.id ? "rotate-180" : ""}`}>
                      ...
                      <div className="px-4 sm:px-8 pb-6 sm:pb-8 pt-2 border-t border-border/20 bg-muted/5">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 py-6">
                          <div className="space-y-2">
                            ...
                            <p className="text-sm font-medium tracking-tight">{getWalletName(tx)}</p>
                          </div>
                          <div className="space-y-2">
                            <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                              <TrendingUp className="w-3 h-3" /> Sektor
                            </p>
                            <p className="text-sm font-medium  tracking-tight">{tx.category}</p>
                          </div>
                          <div className="space-y-2">
                            <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                          ...
                          <div className="space-y-2">
                            ...
                            <p className="text-sm font-medium tracking-tight truncate">{tx.description || "Tidak ada catatan"}</p>
                          </div>
                        </div>
      ...
      {Object.keys(groupedTransactions).length === 0 && (
        <div className="py-32 text-center space-y-6">
          ...
          <div className="space-y-2">
            <h4 className="text-2xl font-black tracking-tighter">Hening di sini.</h4>
            <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Belum ada aktivitas keuangan yang tercatat.</p>
          </div>
        </div>
      )}
          ...
          <DialogHeader className="space-y-4">
            ...
            <div className="text-center">
              ...
              <DialogDescription className="font-bold text-sm text-muted-foreground mt-2">
                Apakah Anda benar-benar ingin melenyapkan catatan transaksi ini? Tindakan ini tidak bisa dibatalkan.
              </DialogDescription>
            </div>
          </DialogHeader>
          <DialogFooter className="gap-3 mt-8 flex-col sm:flex-row">
            <Button variant="ghost" className="flex-1 rounded-full font-black text-[10px] uppercase tracking-widest h-14" onClick={() => setDeleteId(null)}>
              Batalkan
            </Button>
            <Button 
              variant="destructive" 
